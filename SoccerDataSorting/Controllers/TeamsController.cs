...
namespace SoccerDataSorting.Controllers
{
    public class TeamsController : Controller
    {
        ...
        // GET: Teams
        public async Task<IActionResult> Index(SortStateTeam sortOrder = SortStateTeam.NameAsc)
        {
            IQueryable<Teams>? teams = _context.Teams;

            ViewData["NameSort"]= sortOrder==SortStateTeam.NameAsc ? SortStateTeam.NameDesc : SortStateTeam.NameAsc;
            ViewData["CoachSort"]=sortOrder==SortStateTeam.CoachAsc ? SortStateTeam.CoachDesc : SortStateTeam.CoachAsc;

            teams=sortOrder switch
            {
                SortStateTeam.NameDesc => teams.OrderByDescending(t => t.Name),
                SortStateTeam.CoachAsc => teams.OrderBy(t => t.Coach),
                SortStateTeam.CoachDesc => teams.OrderByDescending(t => t.Coach),
                _ => teams.OrderBy(t => t.Name),
            };

            return _context.Teams != null ?
                        View(await teams.ToListAsync()) :
